...
let  customUpdateEvent = (body, callback) =>{
  ...
  if(checkPoint && meetingId) {
    ...
    const event = {
      ...
      'location': recshduleData.g2mMeetingUrl,
...
router.post('/checkuser',(req,res,next)=>{
  console.log(req.body);
  let usernameQuery = "SELECT id,email,userId,fullName,welcomeMessage,profilePic,calendarEvent,token_path,timeZone FROM `calendly` WHERE userId = '" + req.body.userId + "'";
  db.query(usernameQuery, (err, result) => {
    console.log("result=====",result);
    console.log("err=====",err);
    if (err!==null) {
