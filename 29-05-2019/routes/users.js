...
let _conferenceDataVersion = 0;
let _updateEventConferenceDataVersion = 0;

cron.schedule('*/40 * * * *', () => {
  calendarMiddleware.calCheckToken(() => {
...
let  customEvent = (body, callback) =>{
  ...
  let meetingId = typeof (localStorage.getItem("meetingId"))=== 'string' ? localStorage.getItem("meetingId") : '123456789';
  if(checkPoint && meetingId) {
    _conferenceDataVersion = 1;
    const event = {
      'summary': body.userName + ' and ' + body.meetIngData.subject,
      'location': body.data.g2mMeetingUrl,
      description: 'Event Name: ' + body.eventType + '\n'+
        ...
        '<b>Reschedule:</b>' +'\n'+
        'https://cloudmeetin.com/reschedule/'+meetingId + '\n'+
        '<b>Cancel:</b>' +'\n'+
        'https://cloudmeetin.com/cancellations/'+meetingId + '\n'+'\n'+'',
      start: {
    ...
    if (err !== null) {
      ...
      customEvent(req.body, (event) => {
        ...
        calendar.events.insert({
          auth: oauth2Client,
          calendarId: req.body.email,
          conferenceDataVersion: _conferenceDataVersion,
          resource: event,
          "sendUpdates": "all",
        }, function(err, event) {
          if (err) {
...
let  customUpdateEvent = (body, callback) =>{
  ...
  if(checkPoint && meetingId) {
    _updateEventConferenceDataVersion = 1;
    const event = {
    ...
    _updateEventConferenceDataVersion = 0;
    const event = {
      'summary': body.meetIngData.Meeting_owner + ' and ' + body.meetIngData.schedulerName,
      description: 'Event Name: ' + body.meetIngData.eventType + '\n'+
        'Contact Number: ' +body.meetIngData.schedulerPhone+ '\n'+
     ...
     /* const event = {
      ...
      customUpdateEvent(req.body, (event) => {
        ...
        calendar.events.patch({
          ...
          conferenceDataVersion: _updateEventConferenceDataVersion,
          resource: event,
...
function removeEvents(auth, calendar, calendarId, eventID, body, callback){
  ...
  const event = {
    'summary': 'Canceled: ' +body.userName + ' and ' + body.invitee,
...
router.post('/checkuser',(req,res,next)=>{
  console.log(req.body);
  let usernameQuery = "SELECT id,email,userId,fullName,welcomeMessage,profilePic,imageLink,calendarEvent,token_path,timeZone FROM `calendly` WHERE userId = '" + req.body.userId + "'";
  db.query(usernameQuery, (err, result) => {
    console.log("result=====",result);
    console.log("err=====",err);
    if (err!==null) {
      ...
      if(result.length > 0 ) {
        fs.readFile(result[0].welcomeMessage, (err, content) => {
          if (err) console.log('Error loading client secret file:', err);
          let token = JSON.parse(content);
          result[0].welcomeMessage = htmlencode.htmlDecode(token);
          res.status(200).json({
...
router.post('/updateUserProfile',(req,res,next)=>{
  ...
  const TOKEN_PATH = './WelcomeMessage/'+req.body.emailID +'-'+ currentDate +'.json';
  console.log("This is welcome message path  , ", TOKEN_PATH)
  fs.writeFile(TOKEN_PATH, JSON.stringify('Welcome to my scheduling page. Please follow the instructions to add an event to my calendar.'), (err) => {
    if (err) console.error(err);
    let query = "UPDATE `calendly` SET `fullName` = '" + req.body.fullName + "', `password` = '" + req.body.password +"',`welcomeMessage` = '"+TOKEN_PATH+"' WHERE `calendly`.`email` = '" + req.body.emailID + "'";
    db.query(query, (err, result) => {
      console.log("result=====",result);
      console.log("err=====",err);
      if (err!==null) {
...
router.post('/getAvailableDays',(req,res,next)=>{
  ...
  let usernameQuery = "SELECT availableDays,timeZone,fullName,profilePic,imageLink,welcomeMessage,email FROM `calendly` WHERE userId = '"+req.body.userId+"'";
  db.query(usernameQuery, (err, result) => {
    if (err!==null) {
      return res.status(500).send(err);
    }else {
