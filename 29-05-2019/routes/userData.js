...
router.post('/updateProfile', (req, res, next) => {
  ...
  let query = "UPDATE `calendly` SET  `profilePic` = '" + req.body.image + "',  `imageLink` = '" + req.body.webLink +"'  WHERE `calendly`.`userId` = '" + req.body.userId + "'";
...
router.post('/userData', (req, res, next) => {
  ...
  let integrationQuery = "SELECT userId,email,password,fullName,welcomeMessage,language,dateFormat,timeFormat,country,timeZone,profilePic, imageLink FROM `calendly` WHERE userId = '" + req.body.userId + "'";
  db.query(integrationQuery, (err, result) => {
    if (err !== null) {
      console.log('Req', req.body.userID);
      return res.status(500).send(err);
    } else {
      /*  var buffer = new Buffer( result[0].profilePic );
        result[0].profilePic = buffer.toString('base64');*/
      if (result.length > 0) {
        console.log("Path of welcome message is  -- > ", result[0].welcomeMessage);
        fs.readFile(result[0].welcomeMessage, (err, content) => {
          ...
          result[0].welcomeMessage = htmlencode.htmlDecode(token); // result[0].welcomeMessage.toString('base64')
          res.status(200).json({
...
router.post('/addUserData', (req, res, next) => {
  ...
  db.query(integrationQuery, (err, result) => {
    if (result.length > 0) {
      console.log('Result is greater than 1', result[0].welcomeMessage);
      let welcomeMessage = htmlencode.htmlEncode(req.body.welcome);
      console.log('Result is greater than 2 ==== > ', welcomeMessage)
      /*const buf = Buffer.from(JSON.stringify(welcomeMessage));*/
      fs.writeFile(result[0].welcomeMessage, JSON.stringify(welcomeMessage), (err) => {
        if (err) console.error(err);
        let query = "UPDATE `calendly` SET `userId` = '" + req.body.userId + "', `fullName` = '" + req.body.name + "', `language` = '" + req.body.language + "'," + "`dateFormat` = '" + req.body.dateFormat + "' ," + "`timeFormat` = '" + req.body.timeFormat + "' ," + "`country` = '" + req.body.country + "' ," + "`timeZone` = '" + req.body.timeZone + "'  WHERE `calendly`.`userId` = '" + req.body.userId + "'";
        db.query(query, (err, result) => {
        ...
        let query = "INSERT INTO `calendly` ( userId,fullName,welcomeMessage,language,dateFormat,timeFormat,country,timeZone) VALUES ('" + req.body.userId + "', '" + req.body.name + "', '" + TOKEN_PATH + "','" + req.body.language + "','" + req.body.dateFormat + "', '" + req.body.timeFormat + "' , '" + req.body.country + "' , '" + req.body.timeZone + "')";
        db.query(query, (err, result) => {
          console.log("result=====", result);
          console.log("err=====", err);
          if (err !== null) {
