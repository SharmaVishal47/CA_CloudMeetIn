...
function makeBody(to, from, cc,subject, message) {
  ...
  if(checkPoint === 'true') {
   ...
   if(reschedule) {
     ...
     scheduleHTML = '<div>\n' +
       ...
       '        <h3 id="dateAndTime:" style="color: #666a7a; text-decoration: line-through;">  Former: ' +message.rescheduleRecord.meetingTime+ ' to '+message.rescheduleRecord.meetingEndTime+' </h3><br>' +
       '        <h3 id="dateAndTime:" style="color: #666a7a;"> <span style="color: #30bf5b;">Updated: </span>'+new Date(message.starttime)+' to  '+ new Date(message.endtime)+'</h3><br>' +
  ...
  const str = ["Content-Type: text/html; charset=\"UTF-8\"\n",
    ...
    '      <h3 id="eventDateTime" style="color: #666a7a; "> '+new Date(message.starttime)+' to  '+ new Date(message.endtime)+'</h3><br>' +
    '      ' +scheduleHTML +'  \n' +
    '     ' + cancelHTML + ' \n' +
    '    </div>\n' +
    '  </div>\n' +
